...
class MemberRepositoryTest {
    ...
    @Autowired
    // PasswordEncoder : 비밀번호 암호화 해주는 객체
    ...
    public void testRead() {
        // 데이터베이스에서 아이디가 member90 인 데이터의 권한을 찾아서 result에 저장
        Optional<Member> result=memberRepository.getWithRoles("member90");
        // result 값에 오류가 있다면 예외처리
        Member member=result.orElseThrow();
        log.info(member);
        // member90 객체의 설정된 권한 표시
        log.info(member.getRoleSet());
        // 권한이 USER, ADMIN 2개 이므로 2번 반복
