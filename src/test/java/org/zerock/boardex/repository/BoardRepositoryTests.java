...
public class BoardRepositoryTests {
    ...
    public void testInsertWithImage() {
        ...
        // 실제로 연동되어있는 DB에 저장이 된다는 코드
        boardRepository.save(board);
    }
    // 이미지 로딩 (게시물 조회) 테스트
    @Test
    // @Transactional : 필요할 때마다 메서드 내에서 추가적인 쿼리를 여러번 실행할 수 있음
    @Transactional
    public void  testReadWithImage() { // 교수님 메서드명은 testReadWithImages _혹시몰라서 메모
    ...
    public void testModifyImage() {
        //boardRepository 의 findByWithImage 메서드 호출
        Optional<Board> result=boardRepository
                .findByIdWithImage(2L);
        // 게시글 번호(bno)에 해당하는 데이터를 찾아서 board 객체에 저장
        Board board=result.orElseThrow();
        // board 객체의 첨부파일 (이미지) 삭제
        board.clearImage();
        // 수정된 이미지로 변경
        for(int i=0; i<3; i++){
    ...
    // 먼저 Reply 엔티티들을 삭제한 다음 Board 삭제
    @Test
    @Transactional
    @Commit
    ...
    public void testSearchImageReplyCount() {
        // 페이징(현재 페이지 : 0번, 페이지 크기 10ㅇ로 페이징)한 객체를 게시글 번호를 내림차순하여 pageable에 저장
        Pageable pageable=PageRequest.of(0,10,Sort.by("bno").descending());
