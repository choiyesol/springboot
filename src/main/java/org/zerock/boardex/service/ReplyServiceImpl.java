...
@Service
@RequiredArgsConstructor //생성자 있으면 불러와주세요
@Log4j2
public class ReplyServiceImpl implements ReplyService {
    // static : 정적인 변수(객체) 정의할 때 사용
    //         프로그램이 실행될 때 만들어져서 프로그램이 끝날 때까지 없어지지 않고
    //         작동되다가 프로그램이 끝나면 사라지는 변수
    // final : 최종적으로 데이터를 갖고 있는 변수

    // ReplyRepository 객체를 가져와서 replyRepository 라는 메서드로 사용하겠다
    // 한 번 정의하면 값을 변경하지 못하게끔 메서드 앞에 final을 붙여줌
    ...
    public ReplyDTO read(Long rno) {
        Optional<Reply> replyOptional=replyRepository
                .findById(rno);
