...
public class BoardServiceImpl implements BoardService{
    ...
    public BoardDTO readOne(Long bno){
        // board_image 까지 조인 처리하는 findByWithImages() 사용
        // 데이터베이스에서 글번호를 검색해서 그 결과를 result에 저장
        Optional<Board> result=boardRepository.findById(bno);
        //orElseThrow() : result 값이 null 이면 예외처리함.
        Board board=result.orElseThrow();
        //board 객체와 BoardDTO 객체 매핑한 결과(첨부파일과 댓글 포함)를 boardDTO에 저장
        // entityToDTO : BoardService 파일에서 만든 객체
    ...
    public PageResponseDTO<BoardDTO> list(PageRequestDTO pageRequestDTO){
        ...
        //pageRequestDTO 객체에서 키워드 얻어와서 keyword에 저장
        String keyword=pageRequestDTO.getKeyword();
        //글번호를 매개변수로 넘기면서 pageRequestDTO 객체의 getPageable 메서드 호출한 다음 그 결과값을 pageable에 저장
    ...
    @Override
    //PageResponseDTO<T> : Object 타입임.
    public PageResponseDTO<BoardListReplyCountDTO>
    listWithReplyCount(PageRequestDTO pageRequestDTO) {
        // 검색기준(t,c,w) 값을 가져와서 types에 배열로 저장
        // pageRequestDTO 에서 types을 가져와서 types에 배열로 저장해주세요
        String[] types=pageRequestDTO.getTypes();
        // 검색 키워드 가져와서 keyword에 문자열로 저장
        String keyword=pageRequestDTO.getKeyword();
        // 게시글 번호를 기준으로 페이징 된 객체를 pageble에 저장
        Pageable pageable=pageRequestDTO.getPageable("bno");
        // 게시글의 댓글 개수가 포함된 게시판 정보를 result에 저장
        Page<BoardListReplyCountDTO> result=boardRepository.searchWithReplyCount(types,keyword,pageable);
        // 페이징 정보와 검색결과(리스트)와 총 목록 개수(댓글의 총 개수)등의 정보를 반환해주세요
