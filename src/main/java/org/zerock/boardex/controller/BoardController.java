...
public class BoardController {
    ...
    @PreAuthorize("hasRole('USER')") //특정사용자만 권한 설정
    ...
    //게시글 수정하기
    @PreAuthorize("principal.username == #boardDTO.writer") //사전에 권한 설정
    @PostMapping("/modify")
    public String modify(PageRequestDTO pageRequestDTO, @Valid BoardDTO boardDTO, BindingResult bindingResult, RedirectAttributes redirectAttributes){
        ...
        if(bindingResult.hasErrors()) {
            ...
            //bindingResult 객체의 모든 에러를 "errors"라는 이름으로 POST 방식으로 서버에 전송->[[${errors}]]
            redirectAttributes.addFlashAttribute("errors", bindingResult.getAllErrors());
            //데이터의 번호(bno)를 bno라는 이름으로 서버에 POST 방식으로 전송
    ...
    public String remove(BoardDTO boardDTO, RedirectAttributes redirectAttributes){
        ...
        //boardService 객체의 remove메서드 호출하면서 bno(번호)를 넘김
